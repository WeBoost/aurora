...
export function WebsitePreview({ businessId, scale = 1 }: WebsitePreviewProps) {
  const { sections } = useWebsiteContent(businessId);
      ...
      return `
        <section class="py-20 px-4 bg-gray-900">
          <div class="max-w-4xl mx-auto">
            <h2 class="text-4xl font-bold text-center text-white mb-12">${section.content.title}</h2>
            <p class="text-xl text-gray-400 text-center mb-12">${section.content.description}</p>
            <div class="weather-widget" 
                 data-show-aurora="${section.content.showAuroraForecast}"
                 data-show-sun-times="${section.content.showSunTimes}"
                 data-compact="${section.content.compact}">
              <!-- Weather widget will be injected here by JavaScript -->
            </div>
          </div>
        </section>
      `;
